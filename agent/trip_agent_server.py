...
async def get_trip_recommendations(payload: RecommendationRequest):
    ...
    # Create instances of the ChatOpenAIFactory and GooglePlaceAPI
    ...
    interests_list = [interest.strip() for interest in payload.interests.split(",") if interest.strip()]
    ...
    try:
        recommendations = await agent.get_recommendations(payload.n_recommendations, preferences)
        
        # Clean the recommendations data before returning
        cleaned_recommendations = []
        for rec in recommendations:
            rec_dict = rec.model_dump()
            print(f"Recommendation rating: {rec_dict['rating']}")
